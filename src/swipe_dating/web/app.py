...
def _register_discovery_routes(
    ...
    async def discover(request: Request) -> Response:
        ...
        starter_tags = (
            session.visible_starter_tags(candidate.candidate.id) if candidate is not None else ()
        )
        return _render(
            request,
            "discover.html",
    ...
    async def express_interest(
        request: Request,
        candidate_id: Annotated[str, Form()],
        starter_tag: Annotated[str, Form()],
    ) -> Response:
        session = _adult_session(request, sessions)
        if session is None:
            return RedirectResponse("/", status_code=303)
        try:
            outcome = session.express_interest(candidate_id, starter_tag)
        except DomainError as error:
            return _redirect("/discover", error=_domain_message(error))
...
def _domain_message(error: DomainError) -> str:
    messages = {
        ...
        "shared_ground_not_visible": "Choose one of the visible shared-ground starters.",
