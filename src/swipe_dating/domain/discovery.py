"""Mutual-intent eligibility, hard boundaries, transparent rank, and reveal."""
...
def evaluate_discovery_candidate(
    ...
    if exclusions:
        return DiscoveryEvaluation(False, 0, tuple(exclusions), (), "hidden")
    ...
    return DiscoveryEvaluation(True, score, (), explanation, "bio_first")
...
def advance_profile_reveal(current_stage: str, interaction: str) -> str:
    if current_stage == "hidden":
        return "hidden"
    if current_stage == "bio_first" and interaction in {
        "read_bio",
        "inspect_tags",
        "view_explanation",
    }:
