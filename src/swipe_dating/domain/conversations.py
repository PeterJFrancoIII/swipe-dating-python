...
def record_interest(
    ...
    *,
    ...
    normalized_starter = _normalize_tag(starter_tag)
    if not normalized_starter:
        raise DomainError("shared_ground_required")
    at = _to_iso(at_ms)
    decision = Decision(
        id=f"decision-{state.next_event_sequence}",
...
def send_message(
    ...
    *,
    ...
    normalized_shared_ground = _normalize_tag(shared_ground_tag)
    if not local_messages and normalized_shared_ground != match.starter_tag:
        raise DomainError("opening_context_required")
    message = Message(
...
def build_starter_suggestions(match: ConversationMatch) -> tuple[str, ...]:
    ...
    return (
        f"I noticed we both mentioned {label}. What do you enjoy most about it?",
        f"Your {label} tag caught my attention. "
