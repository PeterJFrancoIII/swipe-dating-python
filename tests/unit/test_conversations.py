...
def test_interest_requires_shared_ground_and_candidate_availability() -> None:
    with pytest.raises(DomainError, match="shared_ground_required"):
        record_interest(
            create_conversation_state(),
            candidate=CANDIDATE,
            starter_tag="  ",
    ...
    with pytest.raises(DomainError, match="candidate_already_decided"):
        record_interest(first.state, candidate=CANDIDATE, starter_tag="hiking", at_ms=AT)


def test_first_message_requires_selected_shared_ground() -> None:
    created = matched()
    match_id = str(created.outcome["match_id"])
    with pytest.raises(DomainError, match="opening_context_required"):
        send_message(created.state, match_id=match_id, text="Hello", at_ms=AT)
    with pytest.raises(DomainError, match="opening_context_required"):
        send_message(
            created.state,
            match_id=match_id,
            text="Hello",
            shared_ground_tag="coffee",
            at_ms=AT,
        )
    sent = send_message(
        created.state,
    ...
    assert sent.value.shared_ground_tag == "hiking"
...
def test_starter_suggestions_are_grounded_in_visible_tag() -> None:
    created = matched()
