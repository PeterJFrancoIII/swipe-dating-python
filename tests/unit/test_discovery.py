...
def test_mutual_eligibility_and_required_boundaries_are_hard_gates(
    ...
    assert result.reveal_stage == "hidden"


def test_eligible_candidate_has_explainable_bio_first_score() -> None:
    result = evaluate_discovery_candidate(viewer(), candidate())
    assert result.eligible
    assert 0 <= result.score <= 100
    assert len(result.explanation) == 5
    assert result.reveal_stage == "bio_first"
    assert sum(item.weight for item in result.explanation) == 100


def test_user_weights_change_order_and_ties_use_candidate_id() -> None:
...
def test_progressive_reveal_requires_non_visual_interaction() -> None:
    assert advance_profile_reveal("bio_first", "swipe_right") == "bio_first"
    assert advance_profile_reveal("bio_first", "inspect_tags") == "photo_revealed"
    assert advance_profile_reveal("photo_revealed", "read_bio") == "photo_revealed"
    assert advance_profile_reveal("hidden", "read_bio") == "hidden"
